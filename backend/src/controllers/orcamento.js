...
module.exports = {
    ...
    async store(req, res) {
        ...
        try {
            ...
            if (orcamento) {
                return res.status(400).send({ erro: "Orçamento já cadastrado" })
            }

            orcamento = await cliente.createOrcamento({ data, hora, enderecoPartida, enderecoEntrega, item, descricao })
